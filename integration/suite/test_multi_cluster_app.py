...
from .conftest import (
    ...
    user_project_client, pandaria_create_project_role_template_binding
)
import time
import pytest
import kubernetes
...
def test_multiclusterapp_project_owner_create(admin_mc, admin_pc,
    ...
    mcapp_name = random_str()
    powner = user_factory()
    prtb_owner = pandaria_create_project_role_template_binding(
        client,
        powner.user.id,
        admin_pc.project.id,
        "project-owner")
...
def test_multiclusterapp_user_create(admin_mc, admin_pc, remove_resource,
    ...
    remove_resource(crtb_owner)
    wait_until(rtb_cb(client, crtb_owner))
    prtb_member = pandaria_create_project_role_template_binding(
        client,
        user.user.id,
        admin_pc.project.id,
        "project-member")
...
def test_perform_mca_action_read_only(admin_mc, admin_pc, remove_resource,
    ...
    prtb1 = pandaria_create_project_role_template_binding(
        admin_mc.client,
        user.user.id,
        project.id,
        "read-only")
    ...
    prtb2 = pandaria_create_project_role_template_binding(
        admin_mc.client,
        user_member.user.id,
        project.id,
        "project-member")

...
def pandaria_create_project_role_template_binding(
        admin_client, user_id, project_id, roletemplate_id):
    prtblist = admin_client.list_project_role_template_binding(
        userId=user_id, projectId=project_id, roleTemplateId=roletemplate_id)

    while len(prtblist.data) < 1:
        try:
            prtbname = "prtb-" + random_str()
            prtb = admin_client.create_project_role_template_binding(
                name=prtbname,
                userId=user_id,
                projectId=project_id,
                roleTemplateId=roletemplate_id
            )
            return prtb
        except ApiError as e:
            if "is alreay existed" in e.error.message:
                print("Ignore already existed error")
            else:
                raise e
        prtblist = admin_client.list_project_role_template_binding(
